...
class KArmedBanditTestBed():
    ...
    def constant_step_size_exp_non_stationary_cases(self):
        plt.figure("Constant Step Sizes For Non Stationary Cases")
        plt.title("Constant Step Sizes For Non Stationary Cases")
        ...
        for step_size in self.step_size_values:
            ...
            for run in range(0, self.num_of_runs):
                ...
                for time_step in range(0, self.num_of_time_steps):
                    self.true_q_values += np.random.normal(self.mean, 0.01, self.num_of_bandits)
...
if __name__ == "__main__":
    ...
    k_armed_bandits_test_bed.constant_step_size_exp_non_stationary_cases()